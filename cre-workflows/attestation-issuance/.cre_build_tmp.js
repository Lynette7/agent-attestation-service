...
var configSchema = exports_external.object({
  registryAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address"),
  zkVerifierAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address"),
  performanceApiUrl: exports_external.string().min(1),
...
var TIER_NUMERIC = { STANDARD: 1, VERIFIED: 2 };
var VERIFIED_EXPIRY_SECONDS = 90 * 24 * 60 * 60;
...
function checkEligibility(data, tier) {
  const thresholds = TIER_THRESHOLDS[tier];
  const actualRateBps = Math.floor(data.successCount / data.taskCount * 1e4);
  const eligible = data.taskCount >= thresholds.tasks && actualRateBps >= thresholds.rateBps;
  return { eligible, actualRateBps };
}
var onAttestationRequest = (runtime2, payload) => {
  const request = decodeJson(payload.input);
  runtime2.log(`Attestation request: agent=${request.agent_id.slice(0, 10)}..., tier=${request.tier}`);
  if (request.tier !== "STANDARD" && request.tier !== "VERIFIED") {
    return { success: false, error: `Invalid tier: ${request.tier}` };
  }
  runtime2.log("Fetching performance data via Confidential HTTP...");
  const confHTTPClient = new ClientCapability2;
  const confResponse = confHTTPClient.sendRequest(runtime2, {
    request: {
      url: `${runtime2.config.performanceApiUrl}?agent_id=${request.agent_id}`,
      method: "GET",
      multiHeaders: {
  ...
  if (!ok(confResponse)) {
    ...
    perfData = json(confResponse);
  }
  runtime2.log(`Performance: ${perfData.taskCount} tasks, ${perfData.successCount} successes`);
  const { eligible, actualRateBps } = checkEligibility(perfData, request.tier);
  if (!eligible) {
    const thresholds2 = TIER_THRESHOLDS[request.tier];
    runtime2.log(`Not eligible for ${request.tier}: tasks=${perfData.taskCount}/${thresholds2.tasks}, rate=${actualRateBps}/${thresholds2.rateBps}`);
    return {
      success: false,
      error: `Threshold not met for ${request.tier}`
    };
  }
  runtime2.log(`Eligible for ${request.tier} tier`);
  const thresholds = TIER_THRESHOLDS[request.tier];
  const tierNumeric = TIER_NUMERIC[request.tier];
  const mockProofBytes = "0x" + "00".repeat(128);
  const publicInputs = [
    "0x" + BigInt(thresholds.tasks).toString(16).padStart(64, "0"),
    "0x" + BigInt(thresholds.rateBps).toString(16).padStart(64, "0"),
    "0x" + "0".repeat(64)
  ];
  ...
  if (!network248) {
    return { success: false, error: `Network not found: ${runtime2.config.chainSelectorName}` };
  }
  ...
  runtime2.log("Signed report generated, submitting on-chain...");
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`${request.tier} attestation submitted! TX: ${txHash}`);
    return {
      success: true,
      tx_hash: txHash,
      tier: request.tier
...
var initWorkflow = (config) => {
  ...
  return [
    handler(http2.trigger({}), onAttestationRequest)
  ];
};
async function main() {
